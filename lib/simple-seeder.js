...
module.exports = class SimpleSeeder {
  constructor (store, swarm, opts = {}) {
    ...
    this.store = store.session({ writable: false, manifestVersion: 1 })
    this.swarm = swarm
  ...
  async _createDrive (publicKey, info, opts = {}) {
    const core = this.store.get({ publicKey, compat: false })
    await core.ready()
    const blobs = this.store.get({ publicKey: Hyperdrive.getContentKey(core) })
    await blobs.ready()

    this._suitCore(core, info)
    this._suitCore(blobs, info)
