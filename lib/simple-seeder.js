...
// Hypercore replication can enter in a cascading failure when using
// Hypercore's default approach of dynamically adjusting the range,
// because the CPU can't keep up
// This happens when there are many blocks to download and the core is
// connected to many peers.
// The seeder is susceptible to this error condition,
// so we default to a very conservative fixed range.
// It's a good idea to run some experiments when changing this value
// to ensure your work load does not enter in the error condition
const DEFAULT_INFLIGHT_RANGE = [16, 16]

module.exports = class SimpleSeeder {
  constructor (store, swarm, opts = {}) {
    this.corestore = store

    const inflightRange = opts.inflightRange || DEFAULT_INFLIGHT_RANGE
    this.store = store.session({ writable: false, inflightRange })
