...
const { contentEncoding } = require('seedbee/encoding')
const b4a = require('b4a')

const allowedPeersKey = 'simple-seeder/allowed-peers'

module.exports = class SimpleSeeder extends EventEmitter {
  ...
  async update (list) {
    ...
    try {
      ...
      this.allowedPeers = await list.metadata.get(allowedPeersKey)
  ...
  async _createList (publicKey, info, opts = {}) {
    ...
    this.swarm._firewall = (remotePublicKey) => {
      if (this.allowedPeers.length === 0) {
        this.emit('peer-connected', remotePublicKey)
        return false
      }

      const hexKey = b4a.toString(remotePublicKey, 'hex')
      if (this.allowedPeers.indexOf(hexKey) !== -1) {
        this.emit('peer-connected', remotePublicKey)
        return false
      }

      this.emit('peer-rejected', remotePublicKey)
      return true
    }
